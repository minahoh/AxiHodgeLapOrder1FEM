function [err] = errors_exact_bh_1(p,t,t2,ed,t_e,basis_p2,basis_nd1,x,u_vec_r,u_vec_th,u_vec_z,n)
% ERRORS_EXACT_BH_1 - Calculate the errors of our solution x
% compared to the exact solution u.
%
% Syntax:
%     [err] = 
%         errors_exact_bh_1(p,t,t2,ed,t_e,basis_p2,basis_nd1,x,u_vec_r,u_vec_th,u_vec_z,n)
% Inputs:
...
for T = 1:triangles
    ...
    approx_r = @(r,z) x(t(1,T)).*basis_p2_r(r,z,1) ...
        ...
        + x(nodes+2*edges+2*triangles+t2(1,T)).*basis_p2_r(r,z,4) ...
        + x(nodes+2*edges+2*triangles+t2(2,T)).*basis_p2_r(r,z,5) ...
        + x(nodes+2*edges+2*triangles+t2(3,T)).*basis_p2_r(r,z,6) ...
        + x(nodes+t_e(T,1)).*basis_nd1_r(r,z,1) ...
        + x(nodes+edges+t_e(T,1)).*basis_nd1_r(r,z,2) ...
        + x(nodes+t_e(T,2)).*basis_nd1_r(r,z,3) ...
        + x(nodes+edges+t_e(T,2)).*basis_nd1_r(r,z,4) ...
        + x(nodes+t_e(T,3)).*basis_nd1_r(r,z,5) ...
        + x(nodes+edges+t_e(T,3)).*basis_nd1_r(r,z,6) ...
        + x(nodes+2*edges+T).*basis_nd1_r(r,z,7) ...
        + x(nodes+2*edges+triangles+T).*basis_nd1_r(r,z,8);

    approx_th = @(r,z) x(t(1,T)).*basis_p2_th(r,z,1) ...
        + x(t(2,T)).*basis_p2_th(r,z,2) + x(t(3,T)).*basis_p2_th(r,z,3) ...
        + x(nodes+2*edges+2*triangles+t2(1,T)).*basis_p2_th(r,z,4) ...
        + x(nodes+2*edges+2*triangles+t2(2,T)).*basis_p2_th(r,z,5) ...
        + x(nodes+2*edges+2*triangles+t2(3,T)).*basis_p2_th(r,z,6);
        
    approx_z = @(r,z) x(nodes+t_e(T,1)).*basis_nd1_z(r,z,1) ...
        + x(nodes+edges+t_e(T,1)).*basis_nd1_z(r,z,2) ...
        + x(nodes+t_e(T,2)).*basis_nd1_z(r,z,3) ...
        + x(nodes+edges+t_e(T,2)).*basis_nd1_z(r,z,4) ...
        + x(nodes+t_e(T,3)).*basis_nd1_z(r,z,5) ...
        + x(nodes+edges+t_e(T,3)).*basis_nd1_z(r,z,6) ...
        + x(nodes+2*edges+T).*basis_nd1_z(r,z,7) ...
        + x(nodes+2*edges+triangles+T).*basis_nd1_z(r,z,8);
